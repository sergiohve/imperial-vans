    ...
    <div className="font-sans bg-white text-gray-900">
      <header className="bg-white shadow-md fixed w-full z-50 top-0">
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex flex-col items-center justify-center text-center">
              <h1 className="text-xl md:text-2xl font-bold tracking-tight text-gray-900">
               Lujo en movimiento: Creando interiores personalizados
              </h1>
            </div>
            <div className="flex items-center">
              ...
              <button onClick={toggleMenu} className="lg:hidden text-gray-700 focus:outline-none ml-4">
                <svg
                  className="w-6 h-6"
                  fill="none"
                  stroke="currentColor"
          ...
          <nav className="hidden lg:flex space-x-6 mt-3 justify-center">
            {visibleCategorias.map((item) => (
              <Link
                key={item.id}
                href={`/detail-cars?id=${item.id}`}
                className={`text-[12px] md:text-[12px] font-medium uppercase ${
                  item.id === Number(carId) ? "text-blue-600" : ""
                } tracking-wider hover:text-blue-600`}
              >
                {item.label}
              </Link>
            ))}
            {hiddenCategorias.length > 0 && (
              <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                <PopoverTrigger asChild>
                  <button
                    className={`text-[12px] md:text-[12px] font-medium uppercase tracking-wider hover:text-blue-600 flex items-center ${
                      hiddenCategorias.some(item => item.id === Number(carId)) ? "text-blue-600" : ""
                    }`}
                  >
                    MÁS
                    <svg
                ...
                <PopoverContent className="w-56 bg-white shadow-lg rounded-md p-2 mt-2">
                  <div className="flex flex-col space-y-1">
                    {hiddenCategorias.map((item) => (
                      <Link
                        key={item.id}
                        href={`/detail-cars?id=${item.id}`}
                        className={`block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded ${
                          item.id === Number(carId) ? "text-blue-600 font-semibold" : ""
                        }`}
              ...
              {allCategorias.map((item) => (
                <Link
                  ...
                  className={`text-sm font-medium uppercase ${
                    item.id === Number(carId) ? "text-blue-600" : ""
                  } tracking-wider hover:text-blue-600 whitespace-nowrap`}
                  onClick={toggleMenu}
                >
                  {item.label}
                </Link>
